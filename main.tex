...
security debt accumulation rather than reduction. Platforms such as DefectDojo and Jira
Security Integration provide consolidated dashboards, but without deliberate tool
rationalisation, clearly defined remediation SLAs, and executive accountability, DevSecOps
tooling can paradoxically increase organisational risk by obscuring actionable intelligence
beneath a volume of low-confidence noise.
...
upstream into backlog prioritisation and threat model updates.

\subsection*{Conclusion}

DevSecOps offers a compelling and necessary framework for embedding security throughout the
software delivery lifecycle, but its effectiveness is conditional on the convergence of
cultural transformation, intelligent automation, and continuous operational oversight.
Organisations that treat DevSecOps as a tooling procurement exercise rather than a holistic
security programme remain vulnerable to the same classes of attack that predated its
adoption. Achieving sustained, meaningful security improvement requires addressing the
human, cultural, and architectural dimensions of security alongside the technical,
recognising that security is not a feature to be delivered but a property to be
continuously earned, maintained, and verified.

\vspace{0.5em}
\begin{notebox}
\textit{Approximate word count: 1\,180 words}
\end{notebox}
...
\begin{figure}[H]
  ...
  \node[ctrl=ccode, below=0.1cm of c1]     (c2) {Secret Detection\\(GitLeaks / truffleHog)};
...
\citep{pciDSS2022}, and the OWASP ASVS are documented as acceptance criteria in Jira
tickets, ensuring that security is a first-class functional requirement rather than an
afterthought.
...
and HIPAA, risking regulatory sanction and patient harm. &
...
breach under POPIA Section 22, incurring regulatory penalties and reputational damage. &
...
2 & CSP Header Not Set & \textcolor{medorange}{\textbf{Medium}} & -- &
  Increased XSS/clickjacking exposure due to missing browser policy \\
3 & CSP Directive Missing Fallback & \textcolor{medorange}{\textbf{Medium}} & -- &
  Weak framing controls and reduced defence-in-depth \\
4 & Cross-Domain Misconfiguration (CORS) & \textcolor{medorange}{\textbf{Medium}} & -- &
  Cross-origin data exposure risk if applied to sensitive endpoints \\
5 & Session ID in URL Rewrite & \textcolor{medorange}{\textbf{Medium}} & -- &
